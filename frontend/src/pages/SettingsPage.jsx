...
function DiscoverySettingsSection() {
  ...
  // Parse / toggle helpers for the source pill buttons
  const sourceList = sources.split(',').map(s => s.trim()).filter(Boolean)
  const hasSource  = (s) => sourceList.includes(s)
  const toggleSource = (s) => {
    const next = hasSource(s) ? sourceList.filter(x => x !== s) : [...sourceList, s]
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[
            { key: 'alpaca',   label: 'Alpaca',   icon: '🔵',
              activeClr: 'rgba(59,130,246,.2)', activeBorder: '#3b82f6', activeText: '#60a5fa' },
            { key: 'yfinance', label: 'yfinance', icon: '🟡',
              activeClr: 'rgba(234,179,8,.18)',  activeBorder: '#ca8a04', activeText: '#fbbf24' },
          ].map(({ key, label, icon, activeClr, activeBorder, activeText }) => {
            const active = hasSource(key)
            return (
              <button
                key={key}
                type="button"
                onClick={() => toggleSource(key)}
                title={active ? `Disable ${label}` : `Enable ${label}`}
                style={{
                  display: 'inline-flex', alignItems: 'center', gap: 6,
                  padding: '5px 14px', borderRadius: 20, fontSize: 13,
                  border: `1px solid ${active ? activeBorder : 'var(--border)'}`,
                  background: active ? activeClr : 'transparent',
                  color: active ? activeText : 'var(--text-dim)',
...
export default function SettingsPage({ usage, onUsageRefresh, onHealthRefresh, initialSection = null, onInitialSectionConsumed }) {
  ...
  const [paperProfileNameEnvDefault, setPaperProfileNameEnvDefault] = useState('')
  ...
  const savePaperSettings = async () => {
    ...
    try {
      const body = {
        ...
        profile_name: paperProfileName.trim(),
      }
  ...
  const CLEAR_OPTIONS = [
    ...
    { id: 'paper_orders',        label: 'Paper orders',         desc: 'Local paper trading order cache' },
    { id: 'discovery_runs',      label: 'Discovery runs',       desc: 'Trending discovery run history & candidates' },
    { id: 'watchlist_overrides', label: 'Watchlist overrides',  desc: 'Added/removed tickers — resets to .env base list' },
    { id: 'ticker_memory',       label: 'Ticker memory',        desc: 'AI agent per-ticker context' },
    { id: 'data_cache',          label: 'Data cache',           desc: 'Cached market data (prices, financials, news)' },
    { id: 'backtest_runs',       label: 'Backtest history',     desc: 'All past backtest runs, trades, and profiles' },
  ]

  const toggleClearOption = (id) => {
  ...
  const doClearSelected = async () => {
    setClearConfirm(false)
    setClearStatus('clearing')
    try {
      const r = await fetch(`${API}/settings/clear-data`, {
        method: 'POST',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ categories: [...clearSelected] }),
      })
      if (!r.ok) throw new Error(await r.text())
      const d = await r.json()
      const total = Object.values(d).filter(v => typeof v === 'number').reduce((s, n) => s + n, 0)
      setClearStatus(`ok:${total}`)
  ...
  useEffect(() => {
    Promise.all([
      ...
      setPaperProfileNameEnvDefault(cfg.alpaca_profile_name_env ?? '')
    ...
    <div className="settings-layout">
      ...
      <SettingSection id="settings-paper" title="Paper Trading" icon="📈">
        ...
        <div className="settings-field" style={{ marginBottom: 16 }}>
          <label className="settings-label">Profile / session name
            <span className="text-dim" style={{ fontWeight: 400, marginLeft: 6 }}>(optional — e.g. "Week 38", "Bull run test")</span>
          </label>
          <input
            type="text"
            value={paperProfileName}
            onChange={e => setPaperProfileName(e.target.value)}
            className="settings-select"
            placeholder="e.g. Week 38 — conservative"
            maxLength={80}
        ...
        <button
          ...
          onClick={() => {
            setUseAlpacaEnvDefaults(v => {
              // When toggling ON, pre-fill profile name from env default
              if (!v && paperProfileNameEnvDefault) setPaperProfileName(paperProfileNameEnvDefault)
              return !v
            })
            setAlpacaKeyId('')
            setAlpacaSecret('')
      ...
      <SettingSection id="settings-data" title="Data" icon="🗑️">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
          {CLEAR_OPTIONS.map(({ id, label, desc }) => (
            <label
              key={id}
              style={{
                display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer',
                padding: '8px 12px', borderRadius: 8,
                background: clearSelected.has(id) ? 'rgba(239,68,68,0.08)' : 'var(--surface-2)',
                border: `1px solid ${clearSelected.has(id) ? 'rgba(239,68,68,0.35)' : 'var(--border)'}`,
                transition: 'background 0.15s, border-color 0.15s',
              }}
            >
              <input
                type="checkbox"
                checked={clearSelected.has(id)}
                onChange={() => toggleClearOption(id)}
                style={{ marginTop: 2, accentColor: 'var(--red)', flexShrink: 0 }}
              />
              <span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{label}</span>
                <span className="text-dim" style={{ fontSize: 12, marginLeft: 8 }}>{desc}</span>
              </span>
            </label>
          ))}
        </div>
        ...
        <div className="settings-save-row">
          {clearSelected.size === 0 ? (
            ...
            >
              🗑️ Erase {clearSelected.size} selected
            </button>
          ) : (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
              ...
              <button className="btn-danger btn-sm" onClick={doClearSelected}>Yes, erase</button>
              <button className="btn-sm" onClick={() => setClearConfirm(false)}>Cancel</button>
            </span>
          )}
          {clearStatus === 'clearing' && <span className="text-dim" style={{ fontSize: 12 }}>Erasing…</span>}
          {clearStatus?.startsWith('ok:') && (
            <span className="settings-ok">✓ Done — {clearStatus.split(':')[1]} rows removed</span>
          )}
          {clearStatus === 'error' && <span className="settings-err">✗ Failed — see backend logs</span>}
        </div>

        {/* Legacy full-clear ───────────────────────────────────────────── */}
        <p className="text-dim" style={{ fontSize: 12, marginTop: 20, marginBottom: 6 }}>
          <strong>Quick wipe</strong> — clear all signals &amp; analysis log in one click (legacy):
        </p>
        <div className="settings-save-row">
          <button className="btn-danger btn-sm" onClick={resetData} disabled={resetStatus === 'clearing'}>
            {resetStatus === 'clearing' ? 'Clearing…' : 'Clear all signals & analyses'}
          </button>
