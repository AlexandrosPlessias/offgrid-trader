...
export default function TrendingPage({ onViewChange, onOpenSettings, onOpenExplorer }) {
  ...
  const logRef = useRef(null)   // progress log container — auto-scrolls internally

  // Auto-scroll the step log to the latest entry WITHIN its own box (never the page)
  useEffect(() => {
  ...
  return (
    <div className="page-content" style={{ padding: '24px 32px' }}>
      ...
      {progress.length > 0 && (
        <div ref={logRef} className="discovery-log card" style={{ marginBottom: 16, maxHeight: 240, overflowY: 'auto' }}>
          {progress.map((item, i) => {
                ...
                return (
                  <Fragment key={c.ticker}>
                    <tr style={{ borderBottom: isOpen ? 'none' : '1px solid var(--border-subtle, rgba(255,255,255,.06))' }}>
                      ...
                      <td style={{ ...cell, textAlign: 'center', overflow: 'hidden' }}>
                        {(() => {
                          const src = c.source || ''
                          if (!src) return <span style={{ color: 'var(--text-dim)' }}>—</span>
                          const isAlpaca = src.startsWith('alpaca')
                          // Show only the provider — full source (screener type) is in the tooltip
                          const label = isAlpaca ? 'Alpaca' : 'yfinance'
                          return (
                            <span
                              title={src}
                              style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4,
                                background: isAlpaca ? 'rgba(59,130,246,.15)' : 'rgba(234,179,8,.12)',
                                color: isAlpaca ? '#60a5fa' : '#fbbf24',
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, tableLayout: 'fixed', minWidth: 480 }}>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-dim)' }}>
                    ...
                    <th style={{ textAlign: 'left', padding: '6px 12px' }}>Details</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map(run => {
                    const statusColor  = run.status === 'done' ? 'var(--green)' : run.status === 'error' ? 'var(--red)' : 'var(--text-dim)'
                    const statusLabel  = run.status === 'done' ? '✅ Done' : run.status === 'error' ? '❌' : '⏳ Running'
                    ...
                    return (
                      <Fragment key={run.id}>
                        ...
                        >
                          ...
                          <td style={{ padding: '6px 12px', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                              title={run.error || ''}>
                            {run.error
