...
| **Confidence floor** | Minimum confidence score for a signal to be considered actionable (default: 65). Signals below this are computed but not stored or alerted. Use the backtesting confidence-floor sweep to tune this value empirically. |
...
| **R-multiple** | A trade's profit or loss expressed as a multiple of the initial risk. A stop-out gives R = −1.0; hitting the 2 : 1 target gives R = +2.0. Useful because it is currency-independent and bracket-independent. Formula: `R = (exit − entry) / |entry − stop|` (sign-adjusted for shorts). |
...
| **Saved configuration (profile)** | A named snapshot of all backtesting parameters (confidence floor, ATR multiple, R:R, hold days, wallet settings, cashout R, scan interval). Stored locally in the browser. Load a profile to instantly restore a known-good parameter set for a new date window or ticker universe. |
| **Scan interval** | How often the backtest engine fires a signal-detection pass within each trading day. EOD = once per day at market close (fastest and cheapest). Finer intervals (4h, 1h, 30m, 15m) scan multiple times per day but multiply the LLM call count proportionally. |
| **Confidence floor** | The minimum AI confidence score (0–100 %) a signal must reach to be included in backtesting metrics. Raising the floor keeps only the highest-conviction signals; lowering it includes more signals at the cost of lower average quality. Default: 75 %. |
