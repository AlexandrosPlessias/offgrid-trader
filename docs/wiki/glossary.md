...
| **Confidence floor** | Minimum confidence score for a signal to be considered actionable (default: 65 in live scanning, 75 in backtesting). In live mode: signals below the floor are computed but not stored or alerted. In backtesting: raising the floor keeps only the highest-conviction signals; lowering it includes more signals at the cost of lower average quality. Use the backtesting confidence-floor sweep to tune this value empirically. |
