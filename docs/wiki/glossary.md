...
| Term | Plain English |
|---|---|
| <a id="atr"></a>**ATR (Average True Range)** | A measure of how much price typically moves in one period. True Range for each candle = largest of: (High − Low), \|High − Prev Close\|, \|Low − Prev Close\|. ATR is the N-period (usually 14) rolling average of those values. Higher ATR = more volatile asset. |
...
| **Autonomous trading** | The hands-off loop where scan signals automatically place paper (and optionally fractional) orders without a manual tap. Gated by master switches (`PAPER_TRADING_ENABLED`, `FRAC_TRADING_ENABLED`), confidence floors, a position cap (`PAPER_MAX_POSITIONS`), and the tradability gate. Configured under Settings → Autonomous. See [paper-trading.md](paper-trading.md#autonomous-trading). |
...
| **Bracket order** | A three-part order that submits an entry alongside two exits — a stop-loss and a take-profit — so risk and reward are defined at placement. MarketSage's paper trading and autonomous loop place bracket orders on whole shares; the stop/target are derived from an ATR-based bracket around the signal's entry. |
...
| <a id="confidence-floor"></a>**Confidence Floor** | The minimum Confidence Score a signal must reach before MarketSage fires an alert or places a paper trade. Set it higher to only act on the strongest signals; lower it to catch more opportunities at the cost of more noise. Default: 75 (`CONFIDENCE_FLOOR`), runtime-tunable from Settings → Autonomous. See [settings.md](settings.md). |
...
| <a id="deployment-stage"></a>**Deployment stage** | A recommendation from the AI Review describing how ready a strategy is for live trading. Stages (most to least cautious): `reject` → `research_only` → `paper_trade` → `limited_live_candidate`. OOS evidence and fee-stress results drive the stage. |
...
| **EoD report** | The end-of-day report — a summary of the day's account economics, signals, and orders, narrated by the LLM analyst and delivered to your channels. See [reports.md](reports.md). |
| <a id="expectancy-ci95"></a>**Expectancy CI95** | 95 % confidence interval on the expected R per trade, computed via Wilson-score. Wide interval = sample too small to draw conclusions. A tight CI entirely above zero is a strong indicator of edge. |
| <a id="false-positive-rate"></a>**False-positive rate** | In backtesting: `losses / total_trades`. The proportion of signals that triggered a stop-out. Complements win rate. |
...
| <a id="fee-stress-pass"></a>**Fee stress pass** | Whether the average R-multiple exceeds the estimated round-trip transaction cost (0.05 R — commissions + spread). A strategy that passes fee stress is viable net of realistic costs; one that fails is only profitable on paper. |
...
| <a id="in-sample-is"></a>**In-sample (IS)** | A backtest run on the same date window used to select or tune parameters. Metrics are optimistic — the system implicitly "fit" its settings to this data. Use in-sample runs for exploration only; mark them with the toggle OFF. |
...
| <a id="max-drawdown"></a>**Max drawdown** | In the context of backtesting: the worst peak-to-trough decline of the cumulative R-multiple curve. Reported in R-multiples (not dollars). A max drawdown of 5 R means the system was at one point 5 risk-units below its previous high. |
...
| <a id="out-of-sample-oos"></a>**Out-of-sample (OOS)** | A backtest run on a date window that was never used during parameter tuning. Metrics are an honest estimate of real-world performance. Mark OOS runs with the toggle ON; the AI Review weighs them more heavily and may advance the deployment stage. Run each OOS window only once — a second run after re-tuning makes it in-sample again. |
...
| <a id="r-multiple"></a>**R-multiple** | A trade's profit or loss expressed as a multiple of the initial risk. A stop-out gives R = −1.0; hitting a 2 : 1 target gives R = +2.0. Formula: `R = (exit − entry) / |entry − stop|` (sign-adjusted for shorts). Currency-independent and bracket-independent — aim for ≥ 2R. |
...
| <a id="sharpe-ratio"></a>**Sharpe ratio** | In backtesting (R-domain): `mean(R) / std(R)`. Risk-adjusted return normalised to units of risk. Values above 0.5 over a meaningful sample (50 + trades) are worth examining; above 1.0 is strong. |
...
| **Signal drop mode** | How the tradability gate treats un-actionable signals (`SIGNAL_DROP_MODE`): `untradable` drops only permanently-unactionable signals (default); `strict` also drops transiently-blocked ones (no funds/capacity); `never` keeps every signal and only annotates it. |
...
| **Tradability gate** | A pipeline step that checks each signal against Alpaca asset metadata (tradable / shortable / fractionable) and annotates whether it can be placed as a bracket and/or fractional order. In `untradable`/`strict` signal drop mode it removes signals that can be neither, so the feed isn't cluttered with un-actionable alerts. See [paper-trading.md](paper-trading.md#autonomous-trading). |
