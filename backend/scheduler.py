...
from .database import init_db, save_analysis, save_signal
...
async def scan_watchlist(*, send_alerts: bool = True) -> List[Dict[str, Any]]:
    """Scan every ticker in the configured watchlist concurrently."""

    settings = get_settings()
    tasks = [scan_ticker_async(t, send_alerts=send_alerts) for t in settings.watchlist]
...
class MonitorScheduler:
    ...
    async def _loop(self) -> None:
        ...
        self.running = True
        print(
            f"[scheduler] started; watchlist={settings.watchlist} "
            f"interval={settings.scan_interval_minutes}m"
        )
        try:
            while not self._stop.is_set():
                if is_market_open():
                    print(f"[scheduler] market open — scanning {len(settings.watchlist)} tickers")
                    try:
                        ...
                        print(f"[scheduler] scan complete — {total} actionable signal(s)")
                    except Exception as exc:  # pragma: no cover - defensive
                        print(f"[scheduler] scan error: {exc}")
                else:
                    print("[scheduler] market closed — sleeping")
