...
def _run_gate(monkeypatch, opps, assets, drop_mode="untradable"):
    ...
    monkeypatch.setattr(database, "get_setting", lambda k, d="", *a, **kw: drop_mode
                        if k == "signal_drop_mode" else d)
    monkeypatch.setattr(gate_mod, "get_setting", lambda k, d="", *a, **kw: drop_mode
                        if k == "signal_drop_mode" else d)
    ctx = AgentContext(ticker=opps[0]["ticker"] if opps else "TEST")
    ctx.actionable = [dict(o) for o in opps]
    TradabilityGateSkill().run(ctx)
...
def test_tradability_gate(check, monkeypatch):
    ...
    check("long+fractionable kept with both flags",
          "AAA" in kept_by and kept_by["AAA"]["can_bracket"] and kept_by["AAA"]["can_frac"])
    check("long non-fractionable kept, can_frac False",
    ...
    check("unknown ticker kept permissively",
...
def test_tradability_gate_never_mode(check, monkeypatch):
    ...
    check("never mode keeps everything (annotate only)",
          tickers == {"CCC", "DDD"})
    kept_by = {o["ticker"]: o for o in kept}
    check("never mode still annotates can_bracket False on non-tradable",
